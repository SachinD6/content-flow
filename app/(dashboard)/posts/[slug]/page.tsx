      ...
      <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-700 fade-in pb-24">
        ...
        <div className="space-y-6 pt-4">
          <div className="flex flex-wrap gap-2">
            {post.tags?.map((tag) => (
              <Badge key={tag} variant="secondary" className="bg-[#121319] hover:bg-white/10 text-zinc-400 hover:text-white text-[9px] uppercase font-bold tracking-widest border border-white/10 rounded-[6px]">
                {tag}
              </Badge>
            ))}
            {post.featured && (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-sm text-zinc-400 pt-4">
            <div className="flex items-center gap-3 bg-[#121319] p-2 pr-4 rounded-full border border-white/5">
              ...
              <div className="flex flex-col">
                <span className="font-bold text-white text-[12px]">{post.author?.name || 'Unknown Author'}</span>
                <span className="text-[10px] truncate max-w-[150px]">{post.author?.bio || "Architect"}</span>
              </div>
            </div>

            <div className="flex items-center gap-2 px-1">
              <Clock className="h-3.5 w-3.5" />
              <span className="font-mono text-[11px] tracking-wide mt-0.5">
        ...
        {/* Hero Cover Image */}
        {post.coverImage && (
          <div className="relative w-full h-[400px] overflow-hidden rounded-[16px] shadow-2xl border border-white/5">
            <div className="absolute inset-0 bg-gradient-to-t from-[#0b0c10] to-transparent z-10 h-32 bottom-0" />
            <Image
              src={post.coverImage}
              alt="Cover Image"
              fill
