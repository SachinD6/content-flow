...
import { getAuthPage } from '@/lib/sanity/content';
import type { Page } from '@/lib/sanity/content-types';

const iconMap: Record<string, React.ComponentType<{ className?: string; strokeWidth?: number }>> = {
  rocket: SquareTerminal,
  code: Component,
  layers: Layers,
  zap: GitBranch,
  shield: SquareTerminal,
  users: Component,
};

export default async function SignupPage() {
  const authPage = await getAuthPage() as Page;
  
  const brandName = authPage.brandName ?? 'ContentFlow';
  const tagline = authPage.tagline ?? 'CMS-driven publishing for engineering teams.';
  const features = authPage.features ?? [
    { title: 'API-first delivery architecture', icon: 'code' },
    { title: 'Visual Schema Builder v2.0', icon: 'layers' },
    { title: 'Multi-environment staging', icon: 'rocket' },
  ];
          ...
          <div className="mt-32">
            ...
            <div className="space-y-8">
              {features.map((feature, index) => {
                const IconComponent = iconMap[feature.icon ?? 'rocket'] ?? SquareTerminal;
                return (
                  <div key={index} className="flex items-center gap-5">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/5 border border-white/10">
                      <IconComponent className="h-4 w-4 text-zinc-300" strokeWidth={2.5} />
                    </div>
                    <span className="text-lg text-zinc-300 font-medium tracking-tight">{feature.title}</span>
                  </div>
                );
              })}
            </div>
      ...
      <div className="flex w-full lg:w-1/2 flex-col items-center justify-center relative px-4 sm:px-6">
        ...
        <div className="relative z-10 w-full max-w-[420px] rounded-[20px] sm:rounded-[24px] bg-[#121319] p-6 sm:p-8 shadow-2xl border border-white/5 mt-16 lg:mt-0">
          <div className="flex flex-col items-center text-center mb-6 sm:mb-8">
            ...
            <h2 className="text-xl sm:text-2xl font-semibold tracking-tight text-white">{signupPage.title}</h2>
            <p className="mt-2 text-xs sm:text-[13px] text-zinc-400">{signupPage.subtitle}</p>
          </div>
