...
const navItem = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      options: {
        list: [
          { title: '📊 Layout Dashboard', value: 'layout-dashboard' },
    ...
    defineField({
      ...
      title: 'Group',
      type: 'string',
      description: 'Group name for dashboard sidebar sections (e.g. "Main", "Account", "Help")',
    }),
    defineField({
      name: 'badge',
      title: 'Badge Source',
      type: 'string',
      description: 'Show a dynamic count badge. Options: "posts", "drafts"',
      options: {
        list: [
          { title: 'None', value: 'none' },
          { title: 'Post Count', value: 'posts' },
          { title: 'Draft Count', value: 'drafts' },
        ],
      },
      initialValue: 'none',
    }),
  ...
  preview: {
    select: { title: 'label', href: 'href', page: 'page.title', icon: 'icon' },
    prepare({ title, href, page, icon }) {
      return {
        title: title || 'Untitled link',
        subtitle: `${icon ? `${icon} · ` : ''}${page || href || 'No link set'}`,
      }
    },
  },
})
